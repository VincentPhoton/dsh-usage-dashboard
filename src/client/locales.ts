...
export const zh = {
  ...
  'vision.note': '图片按官方换算规则估算：先按比例缩放（上限 ~800×800、下限 384×384），token 与缩放后面积成正比、每张上限 384；图片归属到随后一次模型调用，按该调用的时段与模型单价估算，实际以接口返回的用量为准。',
  'vision.sessionsTitle': '图片最多的会话',
  'vision.sessionRow': '{images} 张 · ¥{cost}',
  'vision.dailyTitle': '{window} · 每天图片数',
  'vision.barAria': '{date} · {images} 张图片 · 估算 {tokens} tokens',
  'vision.barTitle': '{date} · {images} 张图片 · 估算 {tokens} tokens · ¥{cost}',
...
export const en: Record<LocaleKey, string> = {
  ...
  'vision.note': 'Images are estimated with the official rule: resized keeping aspect ratio (cap ~800×800, floor 384×384), tokens proportional to the resized area with a 384-token cap per image. Each image is attributed to the next model call and priced at that call\'s period and model rate; the provider usage numbers remain the source of truth.',
  'vision.sessionsTitle': 'Sessions with the most images',
  'vision.sessionRow': '{images} images · ¥{cost}',
  'vision.dailyTitle': '{window} · images per day',
  'vision.barAria': '{date} · {images} images · est. {tokens} tokens',
  'vision.barTitle': '{date} · {images} images · est. {tokens} tokens · ¥{cost}',
