...
function memoize(ttlMs, compute) {
  ...
  const start = () => {
    const task = compute().then(
      ...
      (err) => {
        if (state.inflight === task) state.inflight = void 0;
        throw err;
      }
    );
    state.inflight = task;
...
var halved = (rates, halveCacheHit = true) => ({
...
function costUnderPeakEra(timeMs, model, input, cache, output, forceOffPeak = false, nowMs = Date.now()) {
...
function pricingInfo(nowMs) {
  ...
  const row = (model, tier) => ({ model, peak: era.peak[tier], offPeak: halved(era.peak[tier], era.halveCacheHit) });
  return {
    currency: "CNY",
...
var BASELINE_GRACE_MS = 15 * 6e4;
var pad2 = (n) => n < 10 ? `0${n}` : String(n);
var beijingDateKey = (nowMs) => {
  const d = new Date(nowMs + BEIJING_OFFSET_MS2);
  return `${d.getUTCFullYear()}-${pad2(d.getUTCMonth() + 1)}-${pad2(d.getUTCDate())}`;
};
var beijingDayStart = (nowMs) => Math.floor((nowMs + BEIJING_OFFSET_MS2) / DAY_MS) * DAY_MS - BEIJING_OFFSET_MS2;
var isFiniteNumber = (value) => typeof value === "number" && Number.isFinite(value);
function readState(path) {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8"));
    if (typeof parsed.date === "string" && isFiniteNumber(parsed.total) && isFiniteNumber(parsed.toppedUp) && typeof parsed.estimatedBaseline === "boolean" && isFiniteNumber(parsed.sampleAt) && isFiniteNumber(parsed.sampleTotal) && isFiniteNumber(parsed.sampleToppedUp)) {
      return parsed;
    }
  } catch {
  }
...
function trackDailyConsumption(balances, nowMs = Date.now(), statePath = DEFAULT_STATE_PATH) {
  ...
  if (primary === void 0) return { consumed: null, estimated: false };
  ...
  if (state === null || state.date !== date) {
    ...
    const observedNearMidnight = state !== null && state.sampleAt < dayStart && dayStart - state.sampleAt <= BASELINE_GRACE_MS;
    const baseline = observedNearMidnight ? { total: state.sampleTotal, toppedUp: state.sampleToppedUp, estimatedBaseline: false } : { total, toppedUp, estimatedBaseline: true };
...
function createRevisionCache(maxWeight) {
  ...
  return {
    get: (id, revision) => {
      if (revision === void 0 || revision === "") return void 0;
      const entry = entries.get(id);
      if (entry === void 0 || entry.revision !== revision) return void 0;
      entries.delete(id);
      entries.set(id, entry);
      return entry.value;
    },
    set: (id, revision, value, weight) => {
      if (revision === void 0 || revision === "") return;
      drop(id);
...
var SESSION_TOP_N = 6;
var FOLD_CACHE_MAX_RECORDS = 12e4;
var foldCache = createRevisionCache(FOLD_CACHE_MAX_RECORDS);
...
function foldSession(events) {
  const fold = { title: "", records: [], usageRecords: 0, skippedRecords: 0 };
  if (events === void 0) return fold;
  let provider = "";
  let model = "";
  const pendingImages = [];
...
function sessionEntries(headers) {
  ...
  for (const listed of headers) {
    ...
    entries.push({
      id,
      revision: typeof revision === "string" && revision !== "" ? revision : void 0,
  ...
  entries.sort((a, b) => a.delegationDepth - b.delegationDepth || a.createdAt - b.createdAt || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
  return entries;
}
async function foldOf(persistence, entry) {
...
async function revisionOf(persistence, id) {
  if (typeof persistence.stat !== "function") return void 0;
  try {
    const snapshot = await persistence.stat(id);
    const revision = snapshot?.revision;
    return typeof revision === "string" && revision !== "" ? revision : void 0;
  } catch {
    return void 0;
  }
}
async function fetchUsage(persistence, nowMs = Date.now()) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const session = { id: entry.id, title, total: 0, cost: 0, calls: 0, lastActive: 0 };
      const windowSessions = windowAggregates.map(() => ({ id: entry.id, title, total: 0, cost: 0, calls: 0, lastActive: 0 }));
